...
    }
    ...
    // ======================================================================
    // ========== 唯一的修改在此函式 (The only modification is in this function) ==========
    // ======================================================================
...
        updatePageControls();
        ...
        // [FIX] Use setTimeout to wait for the browser's rendering engine to stabilize the layout
        // before we try to calculate element widths. This solves the race condition.
        setTimeout(() => {
            if (drawingCtx && drawingCanvas) drawingCtx.clearRect(0, 0, drawingCanvas.width, drawingCanvas.height);
    ...
                if (availableWidth <= 0) {
                    // Fallback if width is still not available
    ...
                updatePageControls();
            });
        }, 0); // The timeout delay is 0, which is enough to push it to the end of the execution queue.
    }
