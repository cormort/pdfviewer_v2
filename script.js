...
document.addEventListener('DOMContentLoaded', () => {
    if (typeof pdfjsLib === 'undefined') {
        ...
        alert('無法載入 PDF 函式庫。請重新整理頁面或檢查您的網路連線。');
        return;
    }
    ...
    async function loadAndProcessFiles(files) {
            ...
            return new Promise((resolve) => {
                ...
                reader.onload = function() {
                    ...
                    pdfjsLib.getDocument({ data: typedarray, isEvalSupported: false, enableXfa: false }).promise.then(pdf => {
                        ...
                        console.error(`載入 ${file.name} 時發生錯誤:`, reason);
                        resolve(null);
                    });
                };
        ...
        try {
            ...
            console.error('檔案處理期間發生錯誤:', error);
            pdfDocs = [];
            updatePageControls();
        }
    }
    ...
    function renderPage(globalPageNum, highlightPattern = null) {
        ...
        doc.getPage(localPage).then(function(page) {
            ...
            page.render(renderContext).promise.then(() => {
                ...
                console.error(`渲染文件 ${pageInfo.docName} 的第 ${localPage} 頁時發生錯誤: ` + reason);
                pageRendering = false;
                updatePageControls();
            });
        }).catch(reason => {
            console.error(`取得文件 ${pageInfo.docName} 的第 ${localPage} 頁時發生錯誤: ` + reason);
            pageRendering = false;
            updatePageControls();
        });
    }
    ...
    function renderTextLayer(page, viewport, highlightPattern) {
        ...
        }).catch(reason => console.error('渲染文字層時發生錯誤: ' + reason));
    }
    ...
    async function renderThumbnail(docIndex, localPageNum, canvasEl) {
        try {
            ...
            console.error(`為文件 ${docIndex} 第 ${localPageNum} 頁渲染縮圖失敗:`, error);
        }
    }
    ...
    function searchKeyword() {
            ...
            for (let i = 1; i <= doc.numPages; i++) {
                ...
                promises.push(
                    doc.getPage(i).then(p => p.getTextContent().then(textContent => {
                        ...
                        console.warn(`處理搜尋時頁面出錯: 文件 ${pageInfo.docName}, 第 ${i} 頁`, err);
                        return null;
                    })
                );
            }
        ...
        Promise.all(promises).then((allPageResults) => {
            ...
            console.error('搜尋期間發生未預期的錯誤:', err);
            ...
            if(resultsList) resultsList.innerHTML = '<p style="padding: 10px;">搜尋期間發生錯誤。</p>';
            renderPage(currentPage, null);
            updateResultsNav();
        });
    }
